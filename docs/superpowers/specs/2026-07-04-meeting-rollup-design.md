...
Canonically a shared skill destined for `CONTROL/skills/`, but until the control-plane skill sync exists (sub-project 3) it ships the working way: placed under `client-kit/.claude/skills/process-meeting/` so `setup_client` delivers it to every client (client-local, like the hooks). Steps:
