...
- `apply_migration(repo, migration) -> set[str]` (interpret ops idempotently; enforce path containment per 4.4; return the exact set of repo-relative paths the ops touched, both `from` and `to` sides of every move plus created/deleted paths; an empty set means the tree was already in the target state). Committing/pushing exactly those paths is the orchestrator's job (4.3 step 4.1), mirroring how `roll_up` leaves the git transaction to `roll_up_all`.
...
3. **If gated:** write `.control-block` (atomic write) with the reasons, print the BLOCKED notice + fix command, emit the standing policy line, apply nothing, skip roll-up. Return `blocked=True`. The gate blocks the **whole** reconciliation, all-or-nothing: no migrations (not even lower-numbered ungated ones), no skills mirror, no policy reload, no MCP announcement, so `.applied.json` never advances while any pending migration is out of reach.
4. **If clear:** remove any stale `.control-block`, then apply in order. Every `.applied.json` update is an atomic write-then-rename, performed only after that step's side effects are durable, so an interrupted pull resumes cleanly next session:
   1. **structure migrations** (ascending `NNNN`): apply the ops; if `apply_migration` returned a non-empty touched-path set, commit and push **exactly those paths** as a transaction (fetch-rebase-retry via `push_paths`) — never a broad `CONTROL/` pathspec, so unrelated working-tree changes are never bundled and a cross-directory move pushes both sides atomically. On an unrecoverable push conflict, `git reset --hard <remote-tip>` and defer (leave `applied.structure_version` unchanged so it retries next session). Only after the commit lands (or the touched set was empty against an already-migrated tree) advance `applied.structure_version` to that `NNNN`. A migration whose touched paths fall outside the publish allowlist is rejected as malformed (gates rather than pushes an unshareable path).
...
`.applied.json` (per-client record of applied versions) and `.control-block` (gate sentinel) live at the client clone root and must never sync. **The primary guard is that nothing ever stages them:** every write path (`publish`, the migration commit, the roll-up commit) stages an explicit pathspec set (the publish allowlist, or a migration's exact touched paths), never `git add -A`, and neither file is in any of those sets. Defense-in-depth on top of that: they are added to the committed `hive-template/.gitignore` (`/.applied.json`, `/.control-block`, inherited by every clone) and to `.git/info/exclude` by `setup_client`, so even a stray manual `git add -A` would skip them. Both files are written atomically (temp file + `os.replace`) so a crash never corrupts them. (Implementation note: the two `hive-template/.gitignore` lines must be added; today it lists only `/private/` and `.claude/skills-local/`.)
